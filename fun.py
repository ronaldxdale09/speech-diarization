...
class SpeechThread(QThread):
    ...
    def run(self):
        ...
        affinity_matrix = cosine_similarity(cont_embeds, cont_embeds)
        ...
        for i, time in enumerate(times):
            if spectral.labels_[i] != prev_label:
                start_time = str(timedelta(seconds=int(prev_time/sampling_rate))).split(".")[0]
                end_time = str(timedelta(seconds=int(time/sampling_rate))).split(".")[0]
                temp = ["Speaker " + str(prev_label+1), f"({start_time} - {end_time})"]
                labelling.append(tuple(temp))
                prev_time = time
            if i == len(times)-1:
                start_time = str(timedelta(seconds=int(prev_time/sampling_rate))).split(".")[0]
                end_time = str(timedelta(seconds=int(len(audio)/sampling_rate))).split(".")[0]
                temp = ["Speaker " + str(spectral.labels_[i]+1), f"({start_time} - {end_time})"]
                labelling.append(tuple(temp))
        ...
        # Merge all list elements to one big string
